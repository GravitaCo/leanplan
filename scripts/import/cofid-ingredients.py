...
Usage:  python3 scripts/import/cofid-ingredients.py path/to/cofid.json path/to/usda.json
Input:  CoFID 2021 proximates as JSON rows {code, name, k, p, c, f, ...}, all per 100 g
        (exported from the "1.3 Proximates" sheet of the CoFID workbook), and USDA SR Legacy
        (2018-04) rows {code: FDC ID, name, k, p, c, f}, per 100 g, for the USDA_FOODS fallback
        list: foods CoFID has no row or no energy value for (our source rules allow USDA then).
...
FOODS = [
    ...
    ('19-497', 'Bacon, back rashers, raw', 'meat', 25, False),
    ('19-646', 'Bacon, back rashers, fat trimmed, raw', 'meat', 25, False),
    ...
    ('16-387', 'King prawns, raw, peeled', 'fish', 100, True),
    ('16-389', 'King prawns, cooked', 'fish', 100, False),
    ('16-263', 'Squid, raw', 'fish', 100, True),
    ('16-497', 'Mussels, raw, flesh only (weigh without shells)', 'fish', 100, False),
    ...
    # ---- Fats and oils
    ('17-661', 'Butter, unsalted', 'fats', 10, False),
    ('17-655', 'Butter, spreadable', 'fats', 10, False),
...
# USDA SR Legacy fallback, only where CoFID has no match or leaves energy blank (N).
# (FDC ID, Tali name, cat, default serving g, cook). USDA spice carbs include fibre, so kcal sits
# below 4/4/9 of the macros; the energy value is used exactly as USDA gives it.
USDA_FOODS = [
    # ---- Spices and dried herbs (CoFID: energy N)
    ('170923', 'Cumin, ground or seeds', 'sauces', 2, False),
    ('171329', 'Paprika', 'sauces', 2, False),
    ('172231', 'Turmeric, ground', 'sauces', 3, False),
    ('171319', 'Chilli powder', 'sauces', 3, False),
    ('171320', 'Cinnamon, ground', 'sauces', 3, False),
    ('170931', 'Black pepper, ground', 'sauces', 1, False),
    ('170933', 'White pepper, ground', 'sauces', 1, False),
    ('171328', 'Oregano, dried', 'sauces', 1, False),
    ('170922', 'Coriander, ground (seeds)', 'sauces', 2, False),
    ('171326', 'Nutmeg, ground', 'sauces', 1, False),
    ('171315', 'Allspice, ground', 'sauces', 2, False),
    ('170919', 'Cardamom, ground', 'sauces', 2, False),
    ('171321', 'Cloves, ground', 'sauces', 1, False),
    ('171323', 'Fennel seeds', 'sauces', 2, False),
    ('170929', 'Mustard seeds, ground', 'sauces', 2, False),
    # ---- Stock cubes (CoFID: energy N). No USDA vegetable cube.
    ('171563', 'Chicken stock cube, dry (1 cube ~10g)', 'sauces', 10, False),
    ('174566', 'Chicken stock, made with a cube', 'sauces', 250, False),
    ('171561', 'Beef stock cube, dry (1 cube ~6g)', 'sauces', 6, False),
    ('172923', 'Beef stock, made with a cube', 'sauces', 250, False),
    # ---- Not in CoFID
    ('173734', 'Black beans, dried, uncooked', 'eggs', 60, False),
    ('175142', 'Sea bass fillet, raw', 'fish', 140, True),
    ('169742', 'Rice noodles, dried, uncooked', 'grains', 60, False),
    ('174030', 'Beef mince, 10% fat, raw', 'meat', 125, True),
    ('174036', 'Beef mince, 20% fat, raw', 'meat', 125, True),
]
...
def main(path, usda_path):
    cofid = {r['code']: r for r in json.load(open(path))}
    usda = {str(r['code']): r for r in json.load(open(usda_path))}
    base = open('src/core/data/foods.ts').read()
    base_names = {n.lower() for n in re.findall(r'"n": "([^"]*)"', base)}
    base_codes = set(re.findall(r'"src": "((?:cofid|usda):[^"]*)"', base))
    out, audit, seen, codes = [], [], set(), set()
    rows = [('cofid', 'CoFID', cofid) + x for x in FOODS] + [('usda', 'USDA', usda) + x for x in USDA_FOODS]
    for key_src, label, table, code, name, cat, g, cook in rows:
        r = table.get(code)
        assert r, f'{code} not in {label}'
        assert r['k'] not in (None, 'N', 'Tr'), f'{code} {r["name"]}: {label} gives no energy value'
        key = name.lower()
        src = f'{key_src}:{code}'
        assert key not in seen and key not in base_names, f'duplicate name: {name}'
        assert src not in codes, f'code used twice: {src}'
        assert src not in base_codes, f'{src} is already in foods.ts'
        seen.add(key); codes.add(src)
        ...
        food['src'] = src
        out.append(food)
        audit.append({'n': name, 'ref': {'source': label, 'code': code, 'name': r['name'], 'k': k, 'p': p, 'c': c, 'f': f},
