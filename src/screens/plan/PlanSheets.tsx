...
export function PlanEditSheet({ id, onClose }: { id?: string; onClose: () => void }) {
  ...
  return (
    <Sheet title={plan ? 'Edit plan' : 'New plan'} onClose={onClose} right={<button className="navbtn b" onClick={save}>Save</button>}>
      ...
      <input className="sheet-input" value={when} placeholder="I get home from work hungry" onChange={(e) => setWhen(e.target.value)} />
      <div className="lbl">I'll…</div>
      <input className="sheet-input" value={then} placeholder="have a yoghurt before I start cooking" onChange={(e) => setThen(e.target.value)} />
      <div className="lbl">If something gets in the way…</div>
      <input className="sheet-input" value={cope} placeholder="keep protein bars in my bag" onChange={(e) => setCope(e.target.value)} />
...
export function PlanReviewSheet({ onClose }: { onClose: () => void }) {
  ...
  const save = () => { reviewPlans(out); onClose() }
  return (
    <Sheet title="Plan check-in" onClose={onClose} right={<button className="navbtn b" onClick={save}>Done</button>}>
      ...
      {due.map((pl) => (
        <div className="card" key={pl.id}>
          ...
          <div className="sub" style={{ margin: '2px 0 10px' }}>I'll {pl.then}</div>
          <div className="chips">
