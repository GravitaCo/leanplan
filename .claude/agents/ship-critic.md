...
4. **Tali's hard constraints** (auto-fail if violated): localStorage key `leanplan.v1` and Supabase table/column names unchanged; RLS still `auth.uid() = user_id` and never loosened; `src/core/` & `src/data/` stay framework-agnostic; the reverted rotation-schedule is not reintroduced; service-worker `CACHE` bumped if shipped assets changed; Vite `base` stays `/` and `public/CNAME` (app.tali.fit) intact; health data never syncs without consent (`runSync` gate) and the consent screen is never bypassed or pre-ticked; if the change alters what personal data is collected, stored or shared, the privacy policy (`src/core/legal/privacy.ts`) and `docs/compliance/README.md` changed with it; `npm run check:legal` passes before anything merges to `main`.
