...
4. **Tali's hard constraints** (auto-fail if violated): localStorage key `leanplan.v1` and Supabase table/column names unchanged; RLS still `auth.uid() = user_id` and never loosened; `src/core/` & `src/data/` stay framework-agnostic; the reverted rotation-schedule is not reintroduced; service-worker `CACHE` bumped if shipped assets changed; Vite `base` stays `/` and `public/CNAME` (app.tali.fit) intact.
